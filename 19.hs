...
opt _ cache state 0 _ = (cache, last $ take 4 state)
opt mx cache state@[i,j,k,l,m,n,o,p] bu bp@(a,b,c,d,e,f) = case M.lookup (bu,state) cache of
  ...
  where val = maximum next
        ...
        next''' = if (est k o bu-1) > mx && cond then [cla,ore,nul] else [nul]
        est o o' b | b<=0 = 0 | otherwise = (if o+o'>=f then 1*(b-1) else 0) + est (mod (o+o') f) (o'+1) (b-1)
